...
class EventTest < Minitest::Test
    ...
    def test_locked_events
        assert_equal([@e,@e4], Event.locked_events)
        refute_equal([@e, @e4,@e3, @event2], Event.locked_events)
    end

    def test_unlocked_events
        assert_equal([@event2,@e3], Event.unlocked_events)
        refute_equal([@e, @e4, @e3, @event2], Event.unlocked_events)
    end
